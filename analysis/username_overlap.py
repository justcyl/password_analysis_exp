...
输出：
1. analysis/results/username_overlap.csv - 子串出现次数与覆盖率
2. analysis/results/username_overlap_pie.html - Top-N 共享子串占比
"""
...
def main() -> None:
    ...
    dataset_overlap: Dict[str, Dict[str, int]] = {}
    ...
    for record in records:
        ...
        dataset_overlap.setdefault(record.dataset, Counter())
        dataset_overlap[record.dataset]["pairs_total"] += 1

        if exact:
            counts["exact_match"] += 1
            dataset_overlap[record.dataset]["exact_match"] += 1
        if lower:
            counts["lower_match"] += 1
            dataset_overlap[record.dataset]["lower_match"] += 1
            token_counter.update(u_lower & p_lower)
        if lev_match:
            counts["lev1_match"] += 1
            dataset_overlap[record.dataset]["lev1_match"] += 1

    csv_path = RESULTS_DIR / "username_overlap.csv"
    ...
    html_path = RESULTS_DIR / "username_overlap_pie.html"
    ...
    print("=== 全量统计 ===")
    for key in ["pairs_total", "exact_match", "lower_match", "lev1_match"]:
        value = counts.get(key, 0)
        pct = value / counts["pairs_total"] * 100 if key != "pairs_total" and counts["pairs_total"] else 0
        print(f"{key}: {value} ({pct:.2f}%)")

    print("\n=== 分数据集统计 ===")
    for dataset, counter in dataset_overlap.items():
        total = counter.get("pairs_total", 1)
        print(f"[{dataset}] total={total} exact={counter.get('exact_match',0)/total*100:.2f}% "
              f"lower={counter.get('lower_match',0)/total*100:.2f}% lev1={counter.get('lev1_match',0)/total*100:.2f}%")
